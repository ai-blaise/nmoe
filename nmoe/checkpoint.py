...
def load_checkpoint(
    ...
    if getattr(cfg, 'resume', True):
        ...
        path = forced_path if resume_step >= 0 else (checkpointer.path_for_step(step) if step >= 0 else None)
        ...
        if path is not None:
            ...
            if import_bootstrap:
                if rank == 0:
                    print_fn(
                        "[checkpoint] step=0 fast import path: skipping pre-load "
                        "layout/signature consensus checks to avoid 128-rank startup stalls."
                    )
            else:
